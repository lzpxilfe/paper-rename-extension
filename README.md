...
**우선순위 팁**: Chrome은 파일명 결정 이벤트를 가장 최근에 활성화한 확장부터 전달합니다. archreport가 아직 인지하지 못하는 예외 호스트(예: `history.seoul.go.kr`, 대학 도서관 프록시)에서 아주 드물게 이름이 겹치는 경우, `chrome://extensions`에서 **paper-rename을 한 번 다시 로드(새로고침)** 하면 paper-rename이 먼저 처리해 해결됩니다.
...
현재 인용식 렌더링, 파일명 정리, 다운로드 context 매칭, 각 지원처 fixture 기반 메타데이터 추출, JSON-LD 파싱, 도메인 필터링 등을 포함한 **총 89개의 단위 테스트**가 포함되어 있습니다.
