...
### 1. 🎓 전국의 모든 대학교 도서관 프록시 서버 대응 (`https://*/*`)
* 특정 대학교 도서관 프록시 도메인만 수동 지원하던 기존 방식에서 탈피하여, **`https://*/*` 범용 와일드카드 권한**을 사용합니다.
* 사용자가 **전국의 어떤 대학교 도서관 프록시 경로**(`*.openlib.uos.ac.kr`, `*.eproxy.yonsei.ac.kr` 등)를 경유하든 관계없이 확장 프로그램이 누출 없이 로드되며, 도메인 내부에 포함된 학술 DB 식별자(riss, dbpia, kiss 등)를 잡아내어 무설정으로 정상 작동합니다.
* **학술 사이트에서만 실제로 동작합니다**: 프록시 도메인을 미리 열거할 수 없어 매칭 범위만 넓게 잡을 뿐, content 스크립트는 로드 직후 `isAcademicSite()`로 현재 주소를 판정하고 **학술 사이트(프록시 포함)가 아니면 즉시 종료**합니다. 일반 웹서핑 중에는 서지 정보 추출도, 클릭 리스너 등록도 일어나지 않습니다.
...
이 확장은 국가유산 보고서 전용 확장인 **archreport**와 동시에 설치해도 서로 간섭하지 않도록 설계되어 있습니다.

| 다운로드 | paper-rename | archreport | 결과 파일명 |
|---|---|---|---|
| 국가유산 보고서 (e-minwon, heritage/cha/khs, nrich, cihc, iha, digital.khs) | 블랙리스트 도메인 → 즉시 통과 | 파일명 변경 | 보고서 인용식 |
| 학술 논문 (RISS, KCI, DBpia 등) | 파일명 변경 | 학술 호스트 감지 → 통과 | 논문 인용식 |
| e-minwon ZIP 일괄다운로드 | 즉시 통과 | ZIP 취소 후 개별 리네임 | 보고서 인용식 |
| 관련 없는 일반 다운로드 | 통과 | 통과 | 원본 유지 |

* 두 확장의 설정 저장소 키는 완전히 분리되어 있어 설정·기능 간 간섭이 없습니다.
* dCollection 원문 메타데이터 보강 요청에는 3초 타임아웃이 있어 다운로드가 멈추지 않습니다.

### 설치 순서와 무관한 양방향 위임

Chrome은 `onDeterminingFilename` 이벤트를 두 확장 모두에게 전달하고, **파일명을 제시한 확장 중 가장 나중에 설치된 쪽**을 채택합니다. 즉 설치·재로드 순서에 따라 승자가 바뀝니다. 이 때문에 두 확장은 서로에게 파일명을 물어보는 통로를 양쪽으로 열어 둡니다.

| 방향 | 상황 | 동작 |
|---|---|---|
| paper-rename → archreport | paper-rename이 먼저 잡았는데 논문 컨텍스트가 없음 | `arch-report-render-filename`으로 문의해 보고서 파일명을 받아 지정 |
| archreport → paper-rename | archreport가 먼저 잡았는데 학술 논문 다운로드임 | `paper-rename-render-filename`으로 문의해 논문 파일명을 받아 지정 |

paper-rename은 `externally_connectable`에 archreport의 확장 ID를 등록해 두었고, 문의를 받으면 **컨텍스트를 소비하지 않고** 파일명만 만들어 돌려줍니다(두 확장의 리스너가 같은 다운로드에 모두 호출되므로, 여기서 소비하면 자기 리스너가 이름을 못 만들게 됩니다). 국가유산 도메인이거나 학술 다운로드가 아니면 빈 문자열을 돌려주어 개입하지 않습니다.

이 통로가 양쪽 모두 살아 있으면 확장 프로그램을 다시 로드할 필요 없이 항상 올바른 파일명이 나옵니다.

> **archreport 0.1.4 이상이 필요합니다.** 그 이전 버전은 `onMessageExternal` 핸들러가 잘못된 확장 ID를 비교하고 있어 문의를 항상 거절했고, 학술 호스트 판정이 대학 도서관 프록시의 하이픈 인코딩(`riss-kr.proxy.univ.ac.kr`)을 인식하지 못해 프록시 경유 논문에 보고서 파일명이 붙을 수 있었습니다.
